...
def calcSigma(x1, x2,l):
	...
	for ii in range(len(x1)):
		for jj in range(len(x2)):
			delta = x1[ii]-x2[jj]
			numer = np.abs(delta)
			exponent = -0.5*(numer/l)**2
			temp = float(np.exp(exponent))
			# temp = float(np.exp(-0.5*(np.abs(x1[ii]-x2[jj])/l)**2))
			Sigma[ii, jj] = temp
			print type(ii), type(jj), type(x1[ii]), type(x2[jj]), type(l)
			print type(Sigma[ii,jj])
